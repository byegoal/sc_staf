...
class DeviceManagerLinux(DeviceManagerBase):
    def get_local_devices(self):
        output = getstatusoutput('ls /dev/sd*[a-z] /dev/xvd*[a-z0-9] /dev/vd*[a-z0-9] /dev/xvd*[a-z0-9]')
        tmp_device_list = output.split()
        print(tmp_device_list)
...
def main(options):
    ...
    if options.list_devices:
        ...
        encrypt_type_l = "ERASE,PRESERVE"
        print device_str
        if options.encrypt_type==False or options.device_format ==False:
            return retval
        else:
        #parameter passing        
            if options.encrypt_type=="ALL":
                preserve_data_list=['no','yes']
            elif options.encrypt_type=="ERASE":
                preserve_data_list=['no']
            else:
                preserve_data_list=['yes']
            device_format_list = options.device_format.split(',')
            idx = 0
            para_list = []
            for encrypt_type in preserve_data_list:
                for device_format in device_format_list:
                    para_list.append([device_format ,encrypt_type ])
                    idx += 1
        
        #create scprov config
        if options.gen_config_path is None:
            config_file_path = os.path.join(device_mgr.get_sc_root(), 'scprov.ini')
        else:
            config_file_path = os.path.join(options.gen_config_path, 'scprov.ini')
        ...
        tmp_string =str()  
        for device in device_mgr.find_new_devices():
                if idy<= len(para_list):                    
                    if tmp_string[:8]==device.disk_name[:8]: # if /vda,/vda1, /vda2, only leave /vda
                        idy += 1
                        tmp_string=device.disk_name 
                        continue
                    else:
                        msg+='[Device%s]\n' %str(device_num+1)
                        if platform.system() == "Windows":
                            ...
                            msg+='DEVICE_NAME=%s\n' %device.disk_name
                        msg+='MOUNT_POINT=/Auto%s\n' %device.disk_name
                        msg+='FILESYSTEM=%s\n' %para_list[device_num][0]
                        msg+='PRESERVE_DATA=%s\n' %para_list[device_num][1]
                        device_num += 1      
                        tmp_string=device.disk_name  
                        idy += 1  
...
def _handle_options(argv):
    ...
    #action_group.add_option('-s', '--device-type', type='choice', dest='device_type',
     #               default=False, metavar= 'device_type', choices=['boot', 'data'], 
      #              help='Devices type to be encrypted (boot | data)')
    action_group.add_option('-f', '--device-format', type='string', dest='device_format',
                    default=False, metavar= 'device_format',
                    help='Devices format to be encrypted (EXT3 | EXT4 ...)')
    action_group.add_option('-t', '--encrypt-type', type='choice', dest='encrypt_type',
                    default=False, metavar= 'encrypt_type',choices=['ERASE', 'PRESERVE','ALL'], 
                    help='Type to be encrypted (ERASE | PRESERVE | ALL)')
