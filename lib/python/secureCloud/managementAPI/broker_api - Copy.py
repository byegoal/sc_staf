...
class broker_api:
    ...
    def api_key_auth(self):
        ...
        logging.debug("auth_id:%s\nrandom_data:%s" % (auth_id, random_data))
        if not auth_id:
            return False
    ...
    def deleteDevice(self, device_id):
        result = self.sc_request(resource='device/'+device_id, method="delete")
        return result
    ...
    def createRAID(self, raid_data):
        result = self.sc_request(resource='device/raid/create', method="post", data=raid_data)
        return result
    ...
    def encryptRAID(self, raid_id, raid_data):
        result = self.sc_request(resource='device/raid/encrypt/'+raid_id, method="post", data=raid_data)
        return result
