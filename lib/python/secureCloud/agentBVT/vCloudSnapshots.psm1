   ...
   <#
    ...
    PROCESS
    {
        if ($PSCmdlet.ShouldProcess("$($entity.name)"))
        {
            ...
            $response = $request.GetResponse()
            $streamReader = new-object System.IO.StreamReader($response.getResponseStream())
            $raw_xmldata = $streamreader.ReadToEnd()
			[xml]$xmldata = $raw_xmldata
			$streamReader.close()
            $response.close()
			echo  "--debug--"
			
			$current_status = $xmldata.task.status
			$current_href = $xmldata.task.href
			
			echo $current_status
			echo $current_href
			
			while ($current_status -ne "success")
			{	
				echo "wait 10 seconds"
				Start-Sleep -s 10

				$request2 = [System.Net.HttpWebRequest]::Create($current_href)
				<# $request2.Method = "POST" #>
				$request2.Accept = "application/*+xml;version=5.1"
				$request2.Headers.add("x-vcloud-authorization",$global:DefaultCIServers[0].SessionId)


				$response2 = $request2.GetResponse()
				$streamReader2 = new-object System.IO.StreamReader($response2.getResponseStream())
				$raw_xmldata2 = $streamreader2.ReadToEnd()
				[xml]$xmldata2 = $raw_xmldata2
				$streamReader2.close()
				$response2.close()

				$current_status = $xmldata2.task.status
				$current_href = $xmldata2.task.href
