...
def wait_ready(strEndpoint, intTimes=3):
    sleep_time = 5
    logging.info("loop waiting STAF service with sleep %ssecs %s times" % (sleep_time, intTimes))
    for i in range(intTimes):
        logging.debug("loop waiting %ssecs: %s of %s retries" % (sleep_time, str(i+1), intTimes))
        time.sleep(sleep_time)
        p = Popen("staf " + strEndpoint + " ping ping", shell=True,
                  stdin=PIPE, stdout=PIPE)
        try:
            #logging.debug("p.commnuicate() result:")
            logging.debug(p.communicate())
            if p.returncode == 0:   #staf ping got responsed
                return 0
...
def shutdown(strEndpoint, intTimes=3):
    logging.info("shutdonw STAF service")
    for i in range(intTimes):
        logging.debug(str(i) + " time:")
        p = Popen("staf " + strEndpoint + " shutdown shutdown ", shell=True,
                  stdin=PIPE, stdout=PIPE)
        try:
            logging.debug("p.wait result:")
            logging.debug(p.wait())
            if p.wait() == 0:
                return 0
...
def wait_stop(strEndpoint, intTimes=1):
    ...
    for i in range(intTimes):
        p = Popen("staf " + strEndpoint + " ping ping", shell=True, stdin=PIPE, stdout=PIPE)
        if p.wait() != 0:
            return 1
        time.sleep(1.0)
    return 0
