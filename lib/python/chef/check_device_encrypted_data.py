...
def main(options):
    retval = 0
    if options.device_array=="":
        errorLogger.error("Please input devices")
        retval = 1
        return retval
    ...
    for devices in devices_list:
        ...
        data_device_result=secureCloud.agentBVT.testingClient.check_device_encrypted_by_device_guid(server_vm_guid,device_msuid,sc_path,10)
        if data_device_result==0:
            ...
            retval=1
    secureCloud.scAgent.file.write_sctm_report(log_path,"Check Device is Encrypted",retval)
    stafLogger.debug("End of device encryption checking")   
    return retval
...
def _handle_options(argv):
    ...
    action_group.add_option('-d', '--device-array', type='string', dest='device_array',
                    ...
                    help='devices to check. ex: -d "Device1,Device2"')
...
if __name__ == "__main__":  
    __retval__ = 0
    stafLogger.critical('*** START DEVICE ENCRYPTED CHECKING ***\n') 
    if __retval__ == 0:
