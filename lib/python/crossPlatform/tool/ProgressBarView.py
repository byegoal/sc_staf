...
class ProgressBarView:
    ...
    def __init__(self, master=None, orientation='horizontal',
        ...
        self.value = value
        if (fillColor is None) or (background is None) or (labelColor is None):
    ...
    def updateProgress(self, newValue, newMax=None, newColor=None):
        if newMax:
            self.max = newMax
        self.value = newValue
    ...
    def RowPos(self, rowid):
        #self.frame.pack(*args, **kw)
        self.frame.grid(row=rowid, column=2)
...
if __name__ == '__main__':
    ...
    label = Label(
        root, text='Progress bar:', anchor=NW, justify=LEFT, width=30)
