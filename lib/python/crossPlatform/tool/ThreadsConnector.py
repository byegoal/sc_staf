...
class ThreadsConnector:
    ...
    def start(self, group, target, name, machinelist, args, kw):
        """ Create and start calculations thread """
        self.calc_thread = threading.Thread(group, self.wrap_calc,
                                            name, (target, machinelist, args, kw))
        self.calc_thread.start()

    def wrap_calc(self, target, machinelist, args, kw):
        """ Function start() continues, but now in calculations thread """
        log = logging.getLogger(self.__class__.__name__)
        try:
            kw['connector'] = self
            kw['progress'] = self.progress
            kw['machinelist'] = machinelist
            target(*args, **kw)
            ...
            e_seq = traceback.format_exception(
    ...
    def runInGui(self, wnd, conn,
                 group=None, target=None, name=None, machinelist=[], args=(), kwargs={}):
        ...
        self.start(group, target, name, machinelist, args, kwargs)
        wnd.go()
        cl.detach()
...
if __name__ == '__main__':
    ...
    import Tkinter
    root = Tkinter.Tk()
    wnd = ActionWindow2.ActionWindow(root, _('Countdown Calculations'),
