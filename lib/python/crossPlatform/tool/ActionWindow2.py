...
class ActionWindow(tkSimpleDialog.Dialog):
    ...
    def __init__(self, parent, title, text, machinelist=None):
        ...
        try:
            while 1:
                ...
                if code == ThreadsConnector.MESSAGE_LOG:
                    ...
                    self.log.error('Unknown ActionWindow message: %s, %s' %
                                   (code, item))
        except Queue.Empty:
            pass
        self.aw_alarm = self.after(100, self.periodicCheckForMessages)
    ...
    def onProgress(self):
        ...
        (cur, limit, color) = self.progress_ctrl.get()
        print cur, limit, color
        print self.progress_bar
        if len(color) > 0:
            for intIndex in range(0, len(color)):
                if color[intIndex] != '':
                    curval = self.progress_bar[intIndex].value
                    print str("cur value = %d" % curval)
                    if curval + 1 > limit:
                        curval = 1
                    else:
                        curval = curval + 1
                    self.progress_bar[intIndex].updateProgress(
                        curval, limit, color[intIndex])
##    for item in self.progress_bar:
##      if len(color) == 0:
##        item.updateProgress(cur, limit, "green")
##      else:
##        item.updateProgress(cur, limit, color[self.progress_bar.index(item)])
    ...
    def packText(self, master):
        ...
        for client in self.machinelist:
            Tkinter.Label(master, text=client, anchor=Tkinter.NW,
                          justify=Tkinter.LEFT).grid(row=self.getrow())
    ...
    def packProgressBar(self, master):
        ...
        row = 0
        for client in self.machinelist:
            obj = ProgressBarView.ProgressBarView(master)
            self.progress_bar.append(obj)
            obj.RowPos(row)
            row = row + 1

    def packLogWindow(self, master):
        """ Pack log window """
        self.logwnd = ScrolledText.ScrolledText(
            master, width=60, height=12, state=Tkinter.DISABLED)
        #self.logwnd.pack(fill=Tkinter.BOTH, expand=1)
        self.logwnd.grid(row=self.getrow(), columnspan=3)
    ...
    def packStatusText(self, master):
        ...
        #self.status_label.grid(row=self.getrow())
        pass
    ...
    def buttonbox(self):
        ...
        (self.button_ok, self.button_cancel) = self.pack_slaves()[-
                                                                  1].pack_slaves()
    ...
    def fixWindowLayout(self):
        ...
        reqwidth = self.button_ok.winfo_reqwidth(
        ) + self.button_cancel.winfo_reqwidth()
