import logging
import os
from tmstaf import platformUtil
...
def unzipFile(strZipFile, strDestPath, strPassword=""):
    '''
    Windows Platform: Call 7za.exe to decompress the input file
    Linux/Mac Platform: Call unzip to decompress the input file
    ...
    objP = TmProcess(lstCmd)
    strStdout = objP.readStdout()
    intRc = objP.wait()
    if intRc == 1 and platformUtil.isMacTiger():
        #unzip 5.51 in MacOS 10.4.11 will return error 1 in some situation even though unzip file successfully
        logging.warning('bypass error code 1 because current platform is MAC OS Tiger')
        return 0
    if intRc != 0:
...
def zipFolder(strFolder, strArchivePath):
    '''
    Windows Platform: Use 7za.exe to compress a folder. For example: zipFolder('c:\result','result001.zip')
    Linux/Mac Platform: Use zip to compress a folder.
    @param strFolder: target folder to compress
    @param strArchivePath: destination zip file
    ...
    try:
        ...
        logging.debug('Compress command "%s"' % lstCmd)
        objP = TmProcess(lstCmd)
        strStdout = objP.readStdout()
...
def main():
    ...
    if args[0] == 'unzip':
        ...
        if len(args) == 3:
            ...
            logging.info("zip result: %s", intResult)
        else:
            logging.info(strUsage)
    else:
        logging.info(strUsage)
